...
export default function VideoDeconstructPanel() {
  ...
  const [parsePhase, setParsePhase] = useState<'input' | 'parsed' | 'analyzing' | 'done'>('input')

  const agents = [
    { key: 'structure', label: '结构拆解', emoji: '🏗️' },
    { key: 'hook', label: '钩子分析', emoji: '🎣' },
    { key: 'script', label: '文案还原', emoji: '📝' },
    { key: 'visual', label: '视觉分析', emoji: '🎨' },
    { key: 'rhythm', label: '节奏分析', emoji: '🎵' },
    { key: 'replicate', label: '复刻建议', emoji: '📋' },
  ]
  ...
  const handleAnalyze = async () => {
    ...
    if (!hasAgent) {
      showToast('请至少选择一个分析维度', 'error')
      return
    }
    setIsGenerating(true)
    setParsePhase('analyzing')
    try {
      const response = await fetch('/api/v1/video/deconstruct', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!response.ok) {
        const err = await response.json().catch(() => null)
        throw new Error(err?.detail || `分析失败: ${response.status}`)
      }
      const json = await response.json()
      setResult(json)
      setParsePhase('done')
      showToast('视频拆解分析完成！', 'success')
    } catch (err) {
      if (err instanceof DOMException && err.name === 'AbortError') return
  ...
  return (
    <div className="px-8 py-10 rounded-3xl bg-white border border-black/[0.04] shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
      <div className="text-center mb-8">
        ...
        <h2 className="text-[20px] font-semibold text-[#1D1D1F] tracking-tight">视频拆解</h2>
        <p className="text-[14px] text-[#86868B] mt-2 max-w-[480px] mx-auto leading-relaxed">
          粘贴短视频分享链接，自动解析视频信息，AI 拆解爆款结构
        </p>
      </div>
      ...
      <div className="max-w-[680px] mx-auto space-y-6">
        <div>
          ...
          <div className="flex gap-3">
            <input
              ...
              placeholder="粘贴抖音、小红书、B站、快手等平台的视频分享链接..."
        ...
        {videoInfo && (parsePhase === 'parsed' || parsePhase === 'analyzing' || parsePhase === 'done') && (
          <motion.div
        ...
        {videoInfo && parsePhase !== 'analyzing' && (
          isGenerating ? (
            <div className="flex gap-3">
        ...
        {parsePhase === 'analyzing' && isGenerating && (
          <div className="flex gap-3">
            <div className="flex-1 h-[52px] rounded-2xl bg-[#1D1D1F]/70 text-white text-[15px] font-medium flex items-center justify-center gap-2.5">
              <Loader2 className="w-5 h-5 animate-spin" />
              <span>AI 拆解分析中...</span>
        ...
        {parsePhase === 'done' && sections && sections.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[15px] font-semibold text-[#1D1D1F]">拆解结果</h3>
              {videoInfo?.video_url && (
                <a
